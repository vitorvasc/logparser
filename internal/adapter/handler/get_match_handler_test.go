...
func TestGetMatchHandler_GetCompleteReportForAllMatches(t *testing.T) {
	testCases := []struct {
		name          string
		serviceMock   port.GetMatchService
		expected      map[string]*dto.MatchDetails
		...
		{
			name: "given unexpected error when getting match should return error",
			serviceMock: func() port.GetMatchService {
				serviceMock := service.NewGetMatchServiceMock()
				serviceMock.On("GetAllMatches", mock.Anything).
					Return(nil, errors.NewError(defines.UnexpectedErrorCode, "unexpected error")).
					Once()
				return serviceMock
			}(),
			expected:      nil,
			expectedError: errors.NewError(defines.UnexpectedErrorCode, "unexpected error"),
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			handler := NewGetMatchHandler(testCase.serviceMock)
			matchDetails, err := handler.GetCompleteReportForAllMatches()
			require.Equal(t, testCase.expectedError, err)
			require.Equal(t, testCase.expected, matchDetails)
		})
	}
...
func getDefaultGetMatchServiceMock() port.GetMatchService {
	...
	serviceMock.On("GetAllMatches").Return([]*domain.Match{
		{
			ID:         "game_1",
			TotalKills: 5,
			Players: []*domain.Player{
